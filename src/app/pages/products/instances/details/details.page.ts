...
export class DetailsPage implements OnInit {
  ...
  private async autoRefresh() {
    ...
    if (counter > 0 && !this.intervalSet) {
      ...
      this.interval = setInterval(() => {
        ...
        if (this.server) {
          if (
            this.server.state === "starting" ||
            this.server.state === "stopping"
          ) {
            newCounter++;
          }
        } else {
  ...
  public async stopInPlace() {
    const alert = await this.alertController.create({
      ...
      message:
        "</br><strong>This action will ask the kernel to shutdown itself via an ACPI call. " +
  ...
  public async reboot() {
    const alert = await this.alertController.create({
      ...
      message:
        "<strong>Warning!</strong><br/> Powering off or rebooting an instance is similar to pulling " +
